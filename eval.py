...
# ---- travel-specific suite (Global Travel Policy Ver 1.4) — kept as template --
# QUESTIONS = [
#     # ---- plain text lookups ----
...
# Generalist mode: write a suite for whatever PDFs are in docs/ (use the travel
# suite above as the format template — same cats: text/table/synthesis/absent).
QUESTIONS: list[dict] = []
...
def main() -> None:
    ...
    console_safe()
    if not QUESTIONS:
        print("No eval questions defined for the current document set.")
        print("Add entries to QUESTIONS (the commented travel suite shows the format).")
        return
